...
public class Demo14_FamilyFilter {
    public static void main(String[] args) {
        ...
        try {
            ...
            byte[] family2 = Bytes.toBytes("in");
            // 定义特定的比较器
            // ByteArrayComparable comparator = new BinaryComparator(family1);//比较
            ByteArrayComparable comparator = new BinaryPrefixComparator(family2);//前缀比较
            // ByteArrayComparable comparator = new SubstringComparator("inf");//字串比较
            //ByteArrayComparable comparator = new RegexStringComparator("^info$");//正则表达式比较
            // 定义过滤器对象
            Filter filter = new FamilyFilter(CompareOperator.EQUAL, comparator);
