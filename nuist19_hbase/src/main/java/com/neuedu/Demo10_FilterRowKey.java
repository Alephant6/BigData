...
public class Demo10_FilterRowKey {
    public static void main(String[] args) {
        ...
        try {
            ...
            Scan scan = new Scan();
            // 定义rowkey
            // byte[] rk1 = Bytes.toBytes("1002");
            // byte[] rk2 = Bytes.toBytes("100");
            // 定义特定的比较器
            // ByteArrayComparable comparator = new BinaryComparator(rk1);//比较
            // ByteArrayComparable comparator = new BinaryPrefixComparator(rk2);//前缀比较
            // ByteArrayComparable comparator = new SubstringComparator("2");//字串比较
            ByteArrayComparable comparator = new RegexStringComparator("^\\d{4}$");//正则表达式比较
            // 定义过滤器对象
            Filter filter = new RowFilter(CompareOperator.EQUAL, comparator);
            // Filter filter = new RowFilter(CompareOperator.GREATER_OR_EQUAL,comparator);
            // 设置过滤器
