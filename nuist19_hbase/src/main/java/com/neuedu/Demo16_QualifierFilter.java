...
public class Demo16_QualifierFilter {
    public static void main(String[] args) {
        ...
        try {
            ...
            // 定义列名
            byte[] c1 = Bytes.toBytes("age");
            byte[] c2 = Bytes.toBytes("ag");
            // 定义特定的比较器
            // ByteArrayComparable comparator = new BinaryComparator(c1);//比较
            ByteArrayComparable comparator = new BinaryPrefixComparator(c2);//前缀比较
            // ByteArrayComparable comparator = new SubstringComparator("ag");//字串比较
            //ByteArrayComparable comparator = new RegexStringComparator("^age$");//正则表达式比较
            // 定义过滤器对象
            // 仅返回符合条件的行键，满足条件的列，并不是返回所有列
            Filter filter = new QualifierFilter(CompareOperator.EQUAL, comparator);
